...
#define YIELD(retval, context, lnum) \
    (context)->label = lnum;         \
    return (retval);                 \
    label##lnum:
                     ...
                     char *str, char *end_p, int iter_limit, int firstp)
{
    ...
    c->ctx = (MatchingContext1 *)GC_malloc(sizeof(struct MatchingContext1));
    c->ctx2 = (MatchingContext2 *)GC_malloc(sizeof(struct MatchingContext2));
    c->ctx->label = 0;
    c->regex = regex;
    c->n_any = 0;
    c->str = str;
              ...
              regexchar *re, char *str, char *end_p, int firstp)
{
    ...
    while (RE_MODE(c->re) != RE_ENDMARK)
    {
        ...
        case RE_SUBREGEX:
            if (c->sub_ctx == NULL)
            {
                c->sub_ctx = (MatchingContext1 *)GC_malloc(sizeof(struct MatchingContext1));
            }
