...
static wc_ces_info WcCesInfo[] = {
    ...
    {WC_CES_ISO_8859_12, "", "", NULL, NULL, NULL, NULL, NULL},
...
const auto WC_CES_T_INDEX = 0x0000ff;

bool wc_check_ces(CharacterEncodingScheme ces)
{
    auto index = ces & WC_CES_T_INDEX;
    return (index <= WC_CES_END && WcCesInfo[index].id == ces);
}

const wc_ces_info &GetCesInfo(CharacterEncodingScheme ces)
{
    auto index = ces & WC_CES_T_INDEX;
    return WcCesInfo[index];
}

wc_ces_list *wc_get_ces_list(void)
{
    static std::vector<wc_ces_list> list;
    if (list.empty())
    {
        for (auto &info : WcCesInfo)
        {
            if (!info.name)
            {
                continue;
        ...
        std::sort(list.begin(), list.end(), [](auto const &a, auto const &b) {
            return strcasecmp(a.desc, b.desc) < 0;
        });
        list.push_back({
            id : WC_CES_NONE,
            name : nullptr,
