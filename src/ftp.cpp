...
ftp_pasv(FTP *ftp)
{
    ...
    case AF_INET6:
        ...
        if (*p == '\0')
            return -1;
        if (sscanf(++p, "%c%c%c%d%c", &d1, &d2, &d3, &port, &d4) != 5 || d1 != d2 || d1 != d3 || d1 != d4)
            return -1;
...
ftp_read:
    ...
    if (status == 125 || status == 150)
        return newFileStream(current_ftp.data, (FileStreamCloseFunc)closeFTPdata);
